...
import { GetVideogamesByNameInput as UseCaseGetVideogamesByNameInput, GetVideogamesByNameUseCase } from "../../src/useCases/videogamesByName.js";
...
export function getVideogamesByNameHandler(server: FastifyInstance) {
  server.post(`/videogames/search`, async (request, response) => {
    try {
      ...
      const getVideogamesByNameUseCase = new GetVideogamesByNameUseCase(videogamesService);

      const rawBody = request.body;

      const input = getVideogamesByNameInputSchema.parse(rawBody);

      const videogames = await getVideogamesByNameUseCase.getVideogamesByName(toUseCaseInput(input));
...
const toUseCaseInput = (input: GetVideogamesByNameInput): UseCaseGetVideogamesByNameInput => ({
  name: input.name,
});
